...
class Model: Codable {
    ...
    var trace: String {
        didSet {
            NotificationCenter.default.post(name: Notification.Name(rawValue: "ModelChanged"), object: nil)
        }
    }
    ...
    */
    func mismatchFunction(x: Value, y: Value) -> Double? {
        ...
        else if (x.number() != nil && y.number() != nil)
        {
            let xNum = x.number()
            let yNum = y.number()
            return -abs(xNum! - yNum!) / 10
        }
        else if (x.number() != nil || y.number() != nil)
        {
            let xString = x.description
